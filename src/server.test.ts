...
// =============================================================================
// Runtime diagnostic: double question mark detection on 404
// =============================================================================
...
Deno.test({
  ...
  await withServer({ mode: "relaxed" }, async (_server, baseUrl) => {
    ...
    assertEquals(response.status, 404);
    const data = await response.json();
    // Should include a diagnostic about the double-? pattern
    assertExists(data.diagnostics, "404 response should include diagnostics");
    assertEquals(data.diagnostics.length, 1);
    assertEquals(data.diagnostics[0].code, "request-double-question-mark");
    assertExists(data.diagnostics[0].message);
    assertExists(data.diagnostics[0].suggestion);
  });
});
...
Deno.test({
  ...
  await withServer({ mode: "relaxed" }, async (_server, baseUrl) => {
    ...
    assertEquals(response.status, 404);
    const data = await response.json();
    // Should NOT include diagnostics
    assertEquals(data.diagnostics, undefined);
  });
});
